...
import { Smile, Hand, MapPin, Heart, Plus, Check } from 'lucide-react';
...
export default function GoldenFriends({ currentUser, textSize }: GoldenFriendsProps) {
  const [friendsList, setFriendsList] = useState<string[]>(["demo_margaret"]); // start with Margaret as a friend
  ...
  // Filter out the current user if they are logged in as one of the demo seniors
  const displaySeniors = DEMO_SENIORS.filter(s => s.uid !== currentUser.uid);
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border-4 border-[#1A1A1A] shadow-[8px_8px_0px_0px_rgba(26,26,26,1)] h-full">
      ...
      <div className="space-y-5">
        {displaySeniors.length === 0 ? (
          <p className="text-gray-500 italic text-sm text-center">
            You are currently the only senior on this device. Refresh to connect!
          </p>
        ) : (
          displaySeniors.map((senior) => {
              ...
              >
                <div className="flex gap-3 items-start sm:items-center">
                  ...
                  <div>
                    ...
                    <p className={`text-[#1A1A1A] font-bold leading-normal mt-0.5 ${getTextSizeClass('text-xs')}`}>
                      {senior.bio}
                    </p>

                    {/* Shared interests */}
                    <div className="flex flex-wrap gap-1 mt-2">
                      {senior.hobbies.map((hobby) => (
                        <span
                          key={hobby}
